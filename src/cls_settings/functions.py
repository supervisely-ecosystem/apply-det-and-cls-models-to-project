...
def get_tags_list_for_predictions(predictions_row):
    pred_scores_list, pred_classes_list = (
        predictions_row["score"],
        predictions_row["class"],
    )
...
def get_predicted_labels_for_batch(labels_batch, model_session_id, top_n, padding):
    ...
    predictions = g.api.task.send_request(
        model_session_id,
        "inference_batch_ids",
        data={"topn": top_n, "pad": padding, **inference_data},
    )
    ...
    for index, predicted_label in enumerate(predicted_labels):
        if len(labels_batch[index]) == 2:  # labels
            ...
            if item_info.id in g.updated_images_ids:
                ann = dataset.get_ann(
                    item_info.name, project_meta=g.output_project.meta
                )
            else:
            ...
            dataset: sly.Dataset = dsid2dataset[item_info.dataset_id]
            if item_info.id in g.updated_images_ids:
                updated_ann = dataset.get_ann(
                    item_info.name, project_meta=g.output_project.meta
                )
                updated_ann = updated_ann.add_tags(predicted_label)
            else:
                updated_ann = sly.Annotation(
                    img_size=(item_info.height, item_info.width),
                    img_tags=predicted_label,
                )
...
def update_annotations_in_for_loop(state):
    ...
    with card_widgets.labeling_progress(
        message="applying models to project data", total=len(g.images_info)
    ) as pbar:
        for image_info in g.images_info:
            ...
            try:
                ann_pred_res = f.validate_response_errors(
                    g.api.task.send_request(
                        state["det_model_id"],
                        "inference_image_id",
                        data={"image_id": image_info.id, "settings": inf_settings},
            ...
            for label in det_labels:
                if (
                    selected_classes_list is None
                    or label.obj_class.name not in selected_classes_list
                ):
                    labels_to_append.append(label)

            if image_info.id in g.updated_images_ids:
                ann = dataset.get_ann(
                    image_info.name, project_meta=g.output_project.meta
                )
            else:
            ...
            for label_info_to_annotate in f.get_images_to_label(
                image_info, selected_classes_list, det_labels
            ):
                labels_batch.append(label_info_to_annotate)
                sly.logger.debug(f"Working with global batch size {g.batch_size}")
                if len(labels_batch) == g.batch_size:
                    ...
                    update_project_items_by_predicted_labels(
                        labels_batch, predicted_labels
                    )
                    labels_batch = []
...
def label_project(state):
    f.create_output_project(
        input_project_dir=g.project_dir, output_project_dir=g.output_project_dir
    )
    ...
    g.output_project_meta = sly.Project(
        g.output_project_dir, mode=sly.OpenMode.READ
    ).meta
...
def upload_project(state):
    ...
    with card_widgets.labeling_progress(
        message="uploading project", total=g.output_project.total_items * 2
    ) as pbar:
...
def get_bbox_with_padding(rectangle, pad_percent, img_size):
    ...
    if pad_percent > 0:
        sly.logger.debug(
            "before padding",
            extra={"top": top, "left": left, "right": right, "bottom": bottom},
        )
...
def get_cropped_image_url(label_b, state):
    ...
    try:
        ...
        top, left, bottom, right = get_bbox_with_padding(
            rectangle=rectangle, pad_percent=state["padding"], img_size=img_np.shape[:2]
        )
...
def get_images_for_preview(state, img_num):
    ...
    g.output_project_meta = f.get_project_meta_merged_with_model_tags(
        g.project_dir, state
    )
    ...
    labels_batch = []
    for label_info_to_annotate in f.get_images_to_label(
        g.preview_image, selected_classes_list, g.preview_boxes
    ):
        labels_batch.append(label_info_to_annotate)
    ...
    for label_b, predicted_label in zip(labels_batch, predicted_labels):
        img_url = (
            get_cropped_image_url(label_b, state)
            if len(label_b) == 2
            else label_b.path_original
        )
        if img_url is None:
            continue
        ...
        images_for_preview.append(
            {
                "url": get_cropped_image_url(label_b, state)
                if len(label_b) == 2
                else label_b.path_original,
                "title": stringify_label_tags(predicted_label.tags)
                if len(label_b) == 2
                else stringify_label_tags(predicted_label),
            }
        )
