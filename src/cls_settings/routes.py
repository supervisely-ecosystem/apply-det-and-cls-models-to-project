...
@card_widgets.select_cls_settings_button.add_route(
    app=g.app, route=ElementButton.Routes.BUTTON_CLICKED
)
def select_cls_settings_button_clicked(
    state: sly.app.StateJson = Depends(sly.app.StateJson.from_request),
):
    try:
        if (
            state["selectedLabelingMode"] == "Objects"
            and len(state["selectedClasses"]) == 0
        ):
        ...
        raise HTTPException(
            status_code=500,
            detail={"title": "Cannot select preferences", "message": f"{ex}"},
        )
    finally:
...
@card_widgets.start_labeling_button.add_route(
    app=g.app, route=ElementButton.Routes.BUTTON_CLICKED
)
def start_labeling_button_clicked(
    state: sly.app.StateJson = Depends(sly.app.StateJson.from_request),
):
    ...
    try:
        if (
            state["selectedLabelingMode"] == "Objects"
            and len(g.selected_det_for_cls_classes) == 0
        ):
            raise ValueError("Classes not selected")
        ...
        raise HTTPException(
            status_code=500,
            detail={"title": "Cannot start labeling", "message": f"{ex}"},
        )
    finally:
        DataJson()["labelingStarted"] = False

        card_widgets.batch_size_input.disabled = False
...
@card_widgets.reselect_cls_settings_button.add_route(
    app=g.app, route=ElementButton.Routes.BUTTON_CLICKED
)
def reselect_cls_settings_button_clicked(
    state: sly.app.StateJson = Depends(sly.app.StateJson.from_request),
):
...
@card_widgets.preview_cls_results_button.add_route(
    app=g.app, route=ElementButton.Routes.BUTTON_CLICKED
)
def preview_cls_results_button_clicked(
    state: sly.app.StateJson = Depends(sly.app.StateJson.from_request),
):
    ...
    try:
        ...
        if state["selectedLabelingMode"] == "Objects" and (
            g.preview_boxes is None or g.preview_image is None or not g.preview_boxes
        ):
        ...
        images_for_preview = card_functions.get_images_for_preview(
            state=state, img_num=6
        )
        ...
        for current_image in images_for_preview:
            card_widgets.preview_grid_gallery.append(
                image_url=current_image["url"], title=current_image["title"]
            )
        ...
        raise HTTPException(
            status_code=500,
            detail={"title": "Cannot preview results", "message": f"{ex}"},
        )
    finally:
...
@card_widgets.select_all_classes_button.add_route(
    app=g.app, route=ElementButton.Routes.BUTTON_CLICKED
)
...
@card_widgets.deselect_all_classes_button.add_route(
    app=g.app, route=ElementButton.Routes.BUTTON_CLICKED
)
